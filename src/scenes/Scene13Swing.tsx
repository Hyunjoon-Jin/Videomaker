...
export const Scene13Swing: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: "#000" }}>
      {/* 전반: 실제 타격 컨택 — 임팩트로 밀고 들어감 */}
      <Plate img="scenes/S13.png" label="Scene 13" title="타격의 순간" seconds={10} motion="in" amount={0.14} atmosphere={{ tone: "warm", intensity: 0.8 }} />
      ...
      {showBall && (
        <AbsoluteFill
          style={{
            opacity: ballIn,
            transform: `scale(${interpolate(frame, [110, 260], [1.16, 1.0], {
              extrapolateLeft: "clamp",
              extrapolateRight: "clamp",
            })})`,
          }}
        >
          <Plate img="scenes/S13B.png" kenBurns={false} scrimTop={false} scrimBottom={false} />
        </AbsoluteFill>
      )}
