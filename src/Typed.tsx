...
export const Typed: React.FC<{
  ...
  return (
     ...
     * 줄바꿈을 어절 단위로 묶는다.
     *
     * 브라우저는 한글을 글자 단위로 끊는다. 그래서 '돌아가는 길에 사람이
     * 죽었다'가 두 줄이 될 때 마지막 줄에 '다' 한 글자만 남았다. 어절이
     * 잘리면 읽다가 한 번 걸린다. keep-all이면 띄어쓰기에서만 끊는다.
     *
     * 여기에 두는 이유는, 자막을 쓰는 쪽이 매번 기억해야 하는 규칙이면
     * 언젠가 빠지기 때문이다. 글자를 그리는 곳에서 한 번 정한다.
     */
    <span style={{ wordBreak: "keep-all", ...style }}>
      {text.slice(0, n)}
