 ...
 * 남한이 낸 요금은 총액의 35%였다.
 *
 * 남한은 전력수요의 60~66%를 북에서 받고 있었다. 설비의 11.5%와는 다른
 * 값이다 — 앞의 것은 발전소가 어디 있었나이고, 이것이 실제로 얼마를
 * 잃었나다. (국가기록원)
 *
 * 전차가 길 위에 섰고 공장이 멈췄다. 격일제 송전과 전열기 사용 금지가
 * 시행됐다. 모내기철과 겹쳐 관개용 전력이 공급되지 않아 쌀 약 55만 석이
 * 덜 났다.
 *
 * 미국에서 발전함 여덟 척을 들여와 1956년까지 전기를 냈다. 1951년에는
 * 발전함이 총 발전량의 56.4%를 냈다. 절반 넘는 전기가 배에서 나온 것이다.
 *
 * ── 정직하게 ──────────────────────────────────────────
...
export const P_EVENTS: PEvent[] = [
  ...
  //
  // 처음에는 '전차가 섰다 / 공장이 멈췄다'로만 적었는데, 그건 화면이
