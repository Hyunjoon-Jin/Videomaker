...
조건은 '활성 상태의 소유권 주장'이고, 길이는 그것이 차단으로 이어지는지를
정한다. 주장이 풀리면 돌아온다 — "소유권 주장이 해결되면 Shorts 동영상의
시청 및 수익 창출이 가능해집니다."

이 채널이 쓰는 Kevin MacLeod 음원은 본인이 직접 Content ID에 등록해 둔
것이다. 제3자의 도둑 등록을 막으려고 그렇게 했다고 incompetech가 밝히고
있다. 그래서 클레임은 매번 걸리고, 표기 문구는 그것을 막는 것이 아니라
이의 제기를 이기게 해주는 근거다.

다만 이 검사가 근본 대책은 아니다. 같은 안내의 FAQ에 답이 있다 —
"YouTube 오디오 보관함의 로열티 없는 음악을 사용할 수도 있습니다.
로열티 없는 음악에는 Content ID 소유권 주장이 제기되지 않습니다."
클레임이 애초에 안 걸리는 음원으로 갈면 길이 조건이 걸릴 일이 없다.
그쪽이 원인 처리고, 이 검사는 그때까지의 안전 여유다.
