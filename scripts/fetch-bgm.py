...
GONGU = [
    ...
    (
        "public/bgm-sp.wav", "13333445", "Walking",
        "최재규, 한국저작권위원회 CC BY", 46.0, 41.6,
        "같은 시·군의 이 끝에서 저 끝까지 — 걷는 이야기라 제목이 맞물린다. "
        "고르기 4.55로 채택 곡들의 범위(2.9~4.9) 한가운데다. 중역 46%는 "
        "낮은 편이지만 채택 범위(43~88%) 안이다. 중역 81%인 '구름산책'이 "
        "휴대폰에서 더 또렷했는데, 저작권자가 '공유마당'으로만 적혀 있고 "
        "곡의 결이 이 편보다 밝다. 131초 중 46초부터가 편 길이 41.6초에 "
        "고르게 맞는다. "
        "https://gongu.copyright.or.kr/gongu/wrt/wrt/view.do?wrtSn=13333445",
    ),
    (
        "public/bgm-il.wav", "13073799", "Walking",
        "기증저작물(자유이용)", 66.0, 39.8,
        "바다에서 가장 먼 곳 — 원이 조용히 커지다 닿는 그림이라 셈여림이 "
        "센 곡은 안 맞는다. 130초 중 66초부터가 중역 76%로 가장 또렷하다. "
        "고르기 2.69는 채택 범위(2.9~4.9) 바로 아래인데, 16편 River(1.82)와 "
        "같은 이유다 — 배경용으로 평탄하게 만든 곡이 아니라 실제 연주곡이다. "
        "18편에도 「Walking」을 썼지만 다른 곡이다. 포락선 상관이 0.36으로 "
        "겹치지 않는다. 이쪽은 기증저작물이라 표기 의무가 없고, 그래도 "
        "설명란에 밝힌다. "
        "https://gongu.copyright.or.kr/gongu/wrt/wrt/view.do?wrtSn=13073799",
    ),
    (
        "public/bgm-sm.wav", "13312635", "LSMBG1006_도심 속 재즈",
        "주식회사 아이티앤, 한국저작권위원회 CC BY", 28.0, 37.5,
        "가장 작은 시·군·구 — 다섯 곳이 전부 광역시 원도심이다. 부산 중구·"
        "대구 중구·인천 동구·서울 중구·부산 동구. 곡 이름이 그대로 그 자리다. "
        "분위기가 아니라 소재와 맞물린다. 137초 중 28초부터가 중역 49%로 "
        "채택 범위(43~88%) 안이고, 고르기 3.60도 범위(2.9~4.9) 한가운데다. "
        "「빌딩 숲 사이」가 이름은 더 맞는데 중역 42%로 범위 바로 아래고 "
        "고르기 2.31도 아래라 접었다. "
        "CC BY라 설명란 표기가 이용 조건이다. 빼면 라이선스 위반이다. "
        "https://gongu.copyright.or.kr/gongu/wrt/wrt/view.do?wrtSn=13312635",
    ),
    (
        "public/bgm-od.wav", "13243498", "Stay Here",
        "오정석, 한국저작권위원회 CC BY", 74.0, 58.4,
        "오디세우스의 동선 — 칼립소가 붙잡아 둔 7년이 10년의 70%고, 곡 "
        "이름이 「여기 머물라」다. 분위기가 아니라 편과 맞물린다. 공유마당 요약도 '느린 피아노 "
        "연주곡으로 서정적인 느낌을 자아내는 낭만적인 곡'이라 10년 중 "
        "9년 반을 붙잡혀 있던 이 편에 맞는다. 150초 중 74초부터가 중역 95%로 지금까지 "
        "받아 본 후보 가운데 제일 또렷하고, 고르기 4.69도 채택 범위(2.9~4.9) "
        "안이다. 17편 Chaser와 같은 작곡가인데 결이 정반대다 — 그쪽은 추격, "
        "이쪽은 머무름. "
        "CC BY라 설명란 표기가 이용 조건이다. 빼면 라이선스 위반이다. "
        "https://gongu.copyright.or.kr/gongu/wrt/wrt/view.do?wrtSn=13243498",
    ),
]
