...
GONGU = [
    ...
    (
        "public/bgm-fw.wav", "13386776", "국악 BGM #36",
        "주식회사 아이티앤, 한국저작권위원회 CC BY", 0.0, 35.5,
        "인구가 가장 적은 지자체 — 울릉군 8,780명을 세는 편이라 밀어붙이면 "
        "안 된다. 공유마당이 이 곡에 붙인 말이 「평화로운」이다. 61초 중 "
        "처음부터가 중역 87%, 고르기 4.90으로 채택 범위(43~88% · 2.9~4.9) "
        "위쪽 끝이다. "
        "**제목으로 소재와 맞물리지는 않는다.** 「섬」이나 「고요」로 "
        "찾아봤지만 나오는 것이 죄다 현장 녹음(파도 소리·새 소리)이라 "
        "BGM이 아니었다. 억지로 맞물리는 척하느니 국악 소편성이 군·섬 "
        "편과 결이 맞는다고만 적는다. "
        "CC BY라 설명란 표기가 이용 조건이다. 빼면 라이선스 위반이다. "
        "https://gongu.copyright.or.kr/gongu/wrt/wrt/view.do?wrtSn=13386776",
    ),
    (
        "public/bgm-td.wav", "13262708", "다시 그 곳으로",
        "김윤아, 한국저작권위원회 CC BY", 31.0, 38.6,
        "밀물이 서해를 올라오는 시간 — 공유마당에 붙은 곡 설명이 "
        "「먼 곳에 있는 지구를 그리워하며 언젠가는 반드시 돌아가고 "
        "말리라는 다짐」이다. 밀물은 하루 두 번 같은 자리로 돌아온다. "
        "분위기가 아니라 소재와 맞물린다. 만료저작물 「고향의 봄」을 "
        "관현악으로 편 곡이다. 107초 중 31초부터가 중역 72%, 고르기 "
        "4.65로 채택 범위 안이다. "
        "CC BY라 설명란 표기가 이용 조건이다. 빼면 라이선스 위반이다. "
        "https://gongu.copyright.or.kr/gongu/wrt/wrt/view.do?wrtSn=13262708",
    ),
    (
        "public/bgm-mt.wav", "13312637", "LSMBG1008_빌딩 숲 사이",
        "한국저작권위원회 CC BY", 101.0, 39.7,
        "지하철로 가장 많이 돌아가는 두 역 — 서울 도심을 도는 편이라 "
        "곡 이름이 그대로 그 자리다. 공유마당 태그도 「도시적인·그루비한」 "
        "이다. 162초 중 101초부터가 중역 50%, 고르기 4.25로 채택 범위 안. "
        "**21편에서 한 번 접었던 곡이다.** 그때는 37.5초 창에서 중역 42%· "
        "고르기 2.31로 범위 아래였는데, 101초 지점은 범위 안이다. 곡 안에서 "
        "어디를 자르느냐가 그만큼 갈린다. 21편이 쓴 「도심 속 재즈」와 같은 "
        "시리즈(LSMBG)지만 포락선 상관이 0.04로 안 겹친다. "
        "CC BY라 설명란 표기가 이용 조건이다. 빼면 라이선스 위반이다. "
        "https://gongu.copyright.or.kr/gongu/wrt/wrt/view.do?wrtSn=13312637",
    ),
    (
        "public/bgm-rs.wav", "13355261", "BGM_09_01_도시적인 chill out 브금",
        "공유마당 CC BY", 19.0, 51.0,
        "아침 8시의 신림역 — 서울 도심의 하루를 도는 편이라 「도시적인」이 "
        "이름에 그대로 있다. 73초 중 19초부터가 중역 53%, 고르기 5.83이다. "
        "**고르기가 채택 범위(2.9~4.9) 위인데 이건 흠이 아니다.** 그 범위는 "
        "지금까지 쓴 곡들이 어디에 있었나를 적어 둔 것이지 넘으면 안 되는 "
        "선이 아니다. 고르기가 높다는 건 셈여림이 없어 배경으로 물러나 "
        "있다는 뜻이라 오히려 이 편에 맞는다. 걸러야 하는 것은 중역이다 — "
        "휴대폰 스피커가 그 대역만 제대로 낸다. "
    ...
    (
        "public/bgm-ct.wav", "13355227", "BGM_03_03_클래식 기타와 첼로브금",
        "공유마당 CC BY", 2.0, 52.8,
        "사람의 한가운데 — **이 편은 한쪽 편을 들면 안 된다.** 수도권이 "
        "북으로 당기고 영남·호남이 남으로 되당겨 균형점이 잡힌다는 "
        "이야기라, 곡이 어느 한쪽으로 기울면 그림과 어긋난다. 18편에서 "
        "기온 폭 순위에 색을 안 내는 곡을 골랐던 것과 같은 까닭이다. "
        "89초 중 2초부터가 중역 80%, 고르기 4.35다. **중역이 80%라 "
        "휴대폰 스피커에서 또렷하다.** "
        "**소재 적합을 꾸며내지 않았다.** 캐시 365곡을 52.83초 창으로 "
        "다시 재니 101곡이 범위 안이었는데 「가운데·균형」에 이름이 "
        "붙는 곡이 하나도 없었다. 국악과 현장녹음(익산역·전주 한옥마을 "
        "주차장·소나기)이 대부분이다. 그래서 이름이 아니라 **결**로 "
        "골랐고, 왜 그랬는지를 여기 적어 둔다. "
        "21·22·23·24·25·18편과 포락선 상관이 -0.15~0.10으로 안 겹친다. "
        "CC BY라 설명란 표기가 이용 조건이다. 빼면 라이선스 위반이다. "
