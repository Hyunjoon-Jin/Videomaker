...
#
#   (지명, 사건 줄, 권.행, 거쳐 가는 자리, 남은 배, 이름표 자리)
...
BEATS = [
    ("이스마로스",
     ["키코네스의 성을 털자 이웃 부족이 반격하다",
      "각 선박마다 6명의 선원을 잃다"],
     (9, 60), ["트로이", "이스마로스"], 12, None),
    ("로토파고이",
     ["말레아곶에서 폭풍을 만나 9일을 떠밀리다",
      "로토스를 먹은 부하들이 고향을 잊다"],
     (9, 94), ["말레아곶", "키테라", "로토파고이"], 12, None),
    ("키클롭스",
    ...
    ("이타카 코앞",
     ...
     (10, 19), ["아이올리아", "이타카", "아이올리아"], 12, "이타카"),
    ("라이스트리고네스",
     ["식인 거인들이 절벽에서 바위를 던지다",
      "배 11척이 부서지고 1척만 빠져나오다"],
     (10, 132), ["라이스트리고네스"], 1, None),
    ("아이아이에",
    ...
    ("세이렌 · 스킬라",
     ["돛대에 몸을 묶고 세이렌을 지나가다",
      "해협의 괴물 스킬라가 6명을 채가다"],
     (12, 246), ["세이렌", "스킬라"], 1, None),
    ("트리나키아 · 오기기아",
     ["태양신의 소를 잡아먹은 벌로",
      "벼락에 마지막 배가 부서지다",
      "혼자 칼립소의 섬에서 7년을 머물다"],
     (12, 417), ["트리나키아", "오기기아"], 0, None),
]
...
def write_json(books):
    ...
    beats = []
    at = "트로이"
    for title, what, (b, ln), route, ships, pin in BEATS:
        pts = []
        for nm in route:
